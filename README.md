...
* **Resource Efficient**: optimized for laptops using batch "chunking" and multicore parallelization.
* **Build Support**: Defaults to `GRCh38` (hg38), with support for `GRCh37` (hg19).
* **User Friendly**: Includes a progress bar and robust pre-flight checks.
...
The recommended way to run the tool is using the `mloy` wrapper script.

### Basic Syntax
```bash
./mloy [OPTIONS] <INPUT_DIRECTORY_OR_FILE>
```
...
#### Automatically detects .idat files. No manifest required (uses sesame annotation)
```bash
./mloy /path/to/idats/ --cores 4
```

### 2. Genotyping Arrays (Illumina IDAT)
#### Critical: You must provide a CSV manifest mapping probes to chromosomes.
```bash
./mloy /path/to/geno_idats/ --manifest manifests/GSA_v3.csv
```
...
### 3. WGS Data (BAM/CRAM)
#### Requires .bai or .crai indexes to be present for fast access.
```bash
./mloy /path/to/bams/ --build GRCh38
```

### 4. Variant Calls (VCF/BCF)
#### Extracts depth from DP or AD fields. Ideal for gVCFs.
```bash
./mloy /path/to/vcfs/ --type vcf
``` 
